...
HB_FUNC ( TOOLTIP_CUSTOMDRAW )
{
   static HWND s_hwndToolTip = NULL, s_hwndControl = NULL;
   static int s_cont = 0;
   ...
   if ( lpNMTTCustomDraw->nmcd.dwDrawStage == CDDS_PREPAINT ) 
   {
      ...
      if ( hb_parni(2) != -1)
           SetBkColor   (lpNMTTCustomDraw->nmcd.hdc, hb_parni(2));

      if ( hb_parni(3) != -1)
           SetTextColor (lpNMTTCustomDraw->nmcd.hdc, hb_parni(3));

      lpNMTTCustomDraw->uDrawFlags = DT_CALCRECT;

      
      HFONT hFont = (HFONT) HMG_parnl (4);
      HWND hwndToolTip = lpNMTTCustomDraw->nmcd.hdr.hwndFrom;
      HWND hwndControl = (HWND) lpNMTTCustomDraw->nmcd.hdr.idFrom;

      if ( hFont == NULL )
         hFont = GetStockObject( DEFAULT_GUI_FONT );

      SendMessage (hwndToolTip, WM_SETFONT, (WPARAM) hFont, MAKELPARAM (FALSE, 0));

      if( s_hwndToolTip != hwndToolTip || s_hwndControl != hwndControl )
      {  s_hwndToolTip = hwndToolTip;
         s_hwndControl = hwndControl;
         s_cont = 0;
      }
      else
         s_cont++;
      ...
      BOOL IsToolTipMenu = (BOOL) hb_parl (5);

      if ( IsToolTipMenu == FALSE )
      {
         ti.cbSize   = sizeof (TOOLINFO);
         ti.uFlags   = TTF_SUBCLASS|TTF_IDISHWND;
         ti.hwnd     = GetParent ( hwndControl );
         ti.uId      = lpNMTTCustomDraw->nmcd.hdr.idFrom;
         ti.lpszText = cText;
         ...
         ti.uId      = lpNMTTCustomDraw->nmcd.hdr.idFrom;
         ti.lpszText = cText;
         SendMessage (hwndToolTip, TTM_SETTOOLINFO, 0, (LPARAM) &ti);   // force to recalculate the size of tooltip after change font
      }
      else
      {
         HWND hWndMenu = (HWND) HMG_parnl (6);
...
HB_FUNC ( HMG_EDITCONTROLGETSEL )
{
   ...
   if (HB_ISBYREF(3))
       hb_stornl ((LONG) nStart, 3);
   if (HB_ISBYREF(4))
       hb_stornl ((LONG) nEnd, 4);
