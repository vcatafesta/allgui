...
BOOL Array2Rectx( PHB_ITEM aRect, RECT * rc );
BOOL Array2Pointx( PHB_ITEM aPoint, POINT * pt );
...
typedef HRESULT ( WINAPI * fnGetThemeSysFont )( HTHEME hTheme, int iFontId, LOGFONT * plf );
...
HB_FUNC( DRAWTHEMEBACKGROUND )
{
   ...
   Array2Rectx( hb_param( 5, HB_IT_ARRAY ), &pRect );
   Array2Rectx( hb_param( 6, HB_IT_ARRAY ), &pClipRect );
...
HB_FUNC( DRAWTHEMEPARENTBACKGROUND )
{
   ...
   if( HB_ISARRAY( 7 ) )
      Array2Rectx( hb_param( 3, HB_IT_ARRAY ), &pRect );
...
HB_FUNC( PTINRECT )
{
   ...
   if( ( Array2Pointx( hb_param( 1, HB_IT_ARRAY ), &point ) && Array2Rectx( hb_param( 2, HB_IT_ARRAY ), &rect ) ) )
...
PHB_ITEM HB_EXPORT Rect2Arrayx( RECT * rc )
{
...
PHB_ITEM HB_EXPORT Point2Array( POINT * pt )
{
