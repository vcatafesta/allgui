...
FUNCTION Main()
   ...
   DEFINE WINDOW MainWnd;
      ...
      ON KEY ALT+RETURN   ACTION If(IsMaximized(MainWnd.HANDLE), MainWnd.RESTORE, MainWnd.MAXIMIZE)
...
FUNCTION ImageFind(aImg)
   ...
   IF (HB_UTF8Left(cUrl, 7) == cProto1) .or. (HB_UTF8Left(cUrl, 8) == cProto2)
      ...
      IF ! URLDownloadToFile(cUrl, cHtmlFile)
         MsgBox("Can not download:" + CRLF + cUrl, MainWnd.TITLE)
   ...
   ELSE
      ...
      IF ! HB_FileExists(cHtmlFile)
         MsgBox("File does not exist:" + CRLF + cHtmlFile, MainWnd.TITLE)
   ...
   IF Empty(aUrl)
      MsgBox("Not found images at:" + CRLF + cUrl, MainWnd.TITLE)
   ELSE
      ImageClose(aImg)
   ...
   IF ! Empty(cTargetFile)
      ...
      IF ! Empty(cTmpFile)
         IF HB_fCopy(cTmpFile, cTargetFile) != 0
            MsgBox("Can not save file:" + CRLF + cTargetFile, MainWnd.TITLE)
         ENDIF
      ENDIF
   ENDIF
...
FUNCTION ImageDownload(aImg, nPos)
   ...
   IF ! HB_FileExists(cTmpFile)
      IF ! URLDownloadToFile(aImg[nPos][2], cTmpFile)
         cTmpFile := ""
         MsgBox("Can not download image:" + CRLF + aImg[nPos][2], MainWnd.TITLE)
      ENDIF
   ENDIF
...
FUNCTION GetExtension(cName)
   ...
   IF ! ((cExt == "jpg") .or. (cExt == "jpeg") .or. (cExt == "png") .or. (cExt == "gif") .or. (cExt == "bmp"))
      cExt := "jpg"
   ENDIF
